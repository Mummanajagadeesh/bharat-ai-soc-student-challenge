...
for idx in range(len(x_test)):
    ...
    line = (
        f"Image {idx:03}: Pred = {pred} ({label_map[pred]}), "
        f"Actual = {true} ({label_map[true]}) "
        f"{'✅' if pred==true else '❌'} | {t_ms:.3f} ms"
    )
...
summary = [
    ...
    "",
    "Inference Time (ms):",
    f"Min : {times_ms.min():.3f}",
    f"Max : {times_ms.max():.3f}",
    f"Avg : {times_ms.mean():.3f}",
    f"Std : {times_ms.std():.3f}",
]
...
# === SAVE FILE (ADDED) ===
with open("results_txt_fixed.txt", "w") as f:
    f.write("\n".join(log_lines))
