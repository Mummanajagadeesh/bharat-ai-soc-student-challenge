...
void myproject(
    ...
    nnet::pointwise_conv_2d_cl<input_t, layer6_t, config19>(input_layer, layer6_out, w6, b6); // conv2d_2

    nnet::add<layer5_t, layer6_t, layer7_t, config7>(layer5_out, layer6_out, layer7_out); // add
    ...
    nnet::conv_2d_cl<layer10_t, layer11_t, config11>(layer10_out, layer11_out, w11, b11); // conv2d_4

    nnet::relu<layer11_t, layer12_t, relu_config12>(layer11_out, layer12_out); // conv2d_4_relu

    nnet::pointwise_conv_2d_cl<layer8_t, layer13_t, config20>(layer8_out, layer13_out, w13, b13); // conv2d_5

    nnet::add<layer12_t, layer13_t, layer14_t, config14>(layer12_out, layer13_out, layer14_out); // add_1

    nnet::pooling2d_cl<layer14_t, layer15_t, config15>(layer14_out, layer15_out); // max_pooling2d_1

    nnet::global_pooling2d_cl<layer15_t, layer16_t, config16>(layer15_out, layer16_out); // global_average_pooling2d

    nnet::dense<layer16_t, dense_result_t, config17>(layer16_out, layer17_out, w17, b17); // dense

    nnet::softmax<dense_result_t, result_t, softmax_config18>(layer17_out, layer18_out); // dense_softmax
